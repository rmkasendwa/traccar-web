  ...
  return (
    <div className="grid h-full place-items-center bg-slate-100 p-6">
      <div className="max-w-md rounded-2xl border border-red-200 bg-white p-7 text-center shadow-sm">
        ...
        <h2 className="mt-4 text-lg font-bold text-slate-950">{t('errorReportLoad')}</h2>
        <p className="mt-2 text-sm leading-6 text-slate-500">{t('errorReportLoadDescription')}</p>
        <button
          type="button"
          onClick={reset}
        ...
        >
          <RotateCcw size={16} /> {t('sharedTryAgain')}
        </button>
      </div>
    </div>
  );

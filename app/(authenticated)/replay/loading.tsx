...
export default function ReplayLoading() {
  return (
    <main className="relative h-full min-h-0 overflow-hidden bg-slate-200" aria-busy="true">
      ...
      <aside className="absolute inset-x-3 bottom-3 z-30 max-h-[calc(100%-1.5rem)] overflow-hidden rounded-[1.35rem] border border-white/60 bg-white/94 shadow-2xl shadow-slate-950/25 backdrop-blur-xl md:inset-x-auto md:top-3 md:left-3 md:w-88">
          ...
          <div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-1.5">
              {[0, 1, 2, 3, 4, 5].map((item) => (
                <div key={item} className="h-8.5 rounded-lg border border-slate-200 bg-slate-50" />
              ))}
              <div className="col-span-2 h-8.5 rounded-lg border border-slate-200 bg-slate-50" />
            </div>
          </div>

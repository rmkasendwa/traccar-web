    ...
    <div className="mt-1.5 space-y-1">
      {group.items.map((item) => {
        ...
        return (
          ...
          >
            <span className={active ? 'text-white' : 'text-slate-500 group-hover:text-sky-300'}>
              {item.icon}
            </span>
      ...
      {groups.map((group) =>
        group.collapsible ? (
          <details key={group.title} open className="group/navigation">
            <summary className="flex cursor-pointer list-none items-center justify-between rounded-lg px-3 py-1 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-slate-500 hover:text-slate-300">
              {group.title}
          ...
          <section key={group.title}>
            <h2 className="px-3 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-slate-500">
              {group.title}
            </h2>
            {items(group)}
          </section>
...
export default function SectionShell({
  ...
  return (
    <div className="flex h-full min-h-0 bg-slate-100">
      <aside className="hidden h-full w-64 shrink-0 border-r border-slate-800 lg:block">
        {sidebar}
      </aside>
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="shrink-0 border-b border-slate-200 bg-white/95 px-4 py-4 backdrop-blur sm:px-6 lg:px-8 lg:py-5">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setDrawerOpen(true)}
              className="grid h-10 w-10 shrink-0 place-items-center rounded-xl border border-slate-200 text-slate-600 transition hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-500 lg:hidden"

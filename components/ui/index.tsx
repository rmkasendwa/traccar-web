...
export const ListItemButton = forwardRef(
  (
    { className, selected, disabled, alignItems, component: Component = 'div', onClick, ...props },
    ref,
  ) => (
    <Component
      ref={ref}
      role={Component === 'div' ? 'button' : undefined}
      tabIndex={Component === 'div' && !disabled ? 0 : undefined}
      aria-disabled={disabled || undefined}
      className={cn(
        'flex min-h-12 w-full gap-3 px-4 py-2 text-left hover:bg-black/5 aria-disabled:pointer-events-none aria-disabled:opacity-50 dark:hover:bg-white/10',
...
export const ListItemText = ({
  ...
  const Primary = slots?.primary || 'span';
  const Secondary = slots?.secondary || 'span';
  return (
    <span className={cn('min-w-0 flex-1', className)} {...props}>
      <Primary className="block truncate" {...slotProps?.primary}>
        {primary}
      </Primary>
      {secondary && (
        <Secondary
...
export const CardMedia = ({
  ...
  Component === 'img' ? (
    ...
    <Component
      className={cn('block w-full bg-cover bg-center', className)}
